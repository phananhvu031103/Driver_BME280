...
static ssize_t bme280_read(struct file *fptr, char __user *buf, size_t len, loff_t *off){
  ...
  sprintf(tempStr, "%ld.%02ld", tempScaled / 100, tempScaled % 100);
  sprintf(pressureStr, "%ld.%02ld", pressureScaled / 256, pressureScaled % 256);
  sprintf(humidityStr, "%ld.%02ld", humidityScaled / 1024, humidityScaled % 1024);
...
static int init_driver(void){
  ...
  // set t_standby time
  i2c_smbus_write_byte_data(bme280_i2c_client, 0xf5, 0x5 << 5);
  ...
  i2c_smbus_write_byte_data(bme280_i2c_client, 0xf2, (0x5 << 0));

  // set the calibration registers
  read_calibration();

  pr_info("bme280: Module init\n");
